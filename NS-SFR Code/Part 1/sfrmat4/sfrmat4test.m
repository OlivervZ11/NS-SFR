function [status, dat, e, fitme, esf, LSF, del2, Con, Angle, PhTF] = sfrmat4test(io, del, npol, weight, a, ErrorVal, oecfdat)
%******************************************************************
...
%sflag = 1;      % added 8 Oct. 2003 for slope specification
switch nargin
...
nn3=floor(npix *nbin);
[~, err, PhTF] = SFRconlim(c, freq, nn3, dcorr);
% Error in ROI
if isempty(err)
    status=[];
    dat=[];
